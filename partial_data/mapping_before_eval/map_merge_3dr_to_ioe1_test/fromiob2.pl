...
#Convert merged test file from IOB2 to IOE1
#Input: IOB2
#Output: IOE1

#use strict;

open (inFP_ioe1, "<test.toIoe1.vote.3dr.wch100.merge");
my $line_ioe1;
my @ref;
...
for (my $j=0;$j<=$#ioe1;$j++){
    ...
    if ($cur[0] eq "I"){
    	...
    	# "E" if follow(I) is "B" and follow = cur
	if ($flw[0] eq "B" and $flw[1] eq $cur[1]){
    ...
    if ($cur[0] eq "B"){
    	...
    	# "E" if follow(B) is "B" and follow = cur
    	if ($flw[0] eq "B" and $flw[1] eq $cur[1]){
       		$ioe1[$j] = $ioe1_words[0]." ".$ioe1_words[1]." ".$ioe1_words[2]." E-".$cur[1];
