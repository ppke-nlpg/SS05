...
#Convert merged test file from IOB1 to IOE2
#Input: IOB1
#Output: IOE2
...
for (my $j=0;$j<=$#ioe2;$j++){
    ...
    if ($cur[0] eq "I"){
    	...
    	# "I" if follow(I) is "I" and follow = cur
    ...
    if ($cur[0] eq "B"){
    	...
    	# "I" if follow(B) is "I" and follow = cur
    	if ($flw[0] eq "I" and $flw[1] eq $cur[1]){
    		$ioe2[$j] = $ioe2_words[0]." ".$ioe2_words[1]." ".$ioe2_words[2]." I-".$cur[1];
    	
    	# "E" if follow(B) is "O" or "B" or empty
    	}elsif ($flw[0]=~/^O|B|$/){
    		$ioe2[$j] = $ioe2_words[0]." ".$ioe2_words[1]." ".$ioe2_words[2]." E-".$cur[1];
    	
    	# "E" if follow(B) is "I" and follow != cur
    	}elsif ($flw[0] eq "I" and $flw[1] ne $cur[1]){
    		$ioe2[$j] = $ioe2_words[0]." ".$ioe2_words[1]." ".$ioe2_words[2]." E-".$cur[1];
    	}
    }    
